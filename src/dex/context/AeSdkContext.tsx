...
import { transactionsQueueAtom } from "../store/txQueueAtoms";
...
export const AeSdkContext = createContext<{
  ...
  setTransactionsQueue: (
    queue: Record<
      string,
      { status: string; tx: Encoded.Transaction; signUrl: string }
    >
  ) => void;
  initSdk: () => void;
...
export const AeSdkProvider = ({ children }: { children: React.ReactNode }) => {
  ...
  const [activeAccount, setActiveAccount] = useAtom<string | undefined>(
    activeAccountAtom
  );
  ...
  const [transactionsQueue, setTransactionsQueue] = useAtom(
  ...
  async function scanForAccounts() {
    ...
    console.log(
      "[AeSdkProvider] scanForAccounts currentAddress",
      currentAddress
    );
