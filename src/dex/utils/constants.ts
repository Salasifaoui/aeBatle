...
import { ICurrency, INetwork } from "../utils/types";
...
// Web-only checks with safe fallbacks for React Native
...
export const IS_FRAMED_AEPP = isWeb && windowObj ? windowObj.parent !== windowObj : false;
export const IS_MOBILE = isWeb && userAgent
  ? userAgent.includes("Mobi")
  : Platform.OS !== "web";
export const IS_SAFARI = isWeb && userAgent
  ? /Mozilla\/5.0 \((Macintosh|iPad|iPhone|iPod); [\s\S]+?\) AppleWebKit\/\S+ \(KHTML, like Gecko\)( (Version|Safari|Mobile)\/\S+)+/.test(userAgent)
  : false;
