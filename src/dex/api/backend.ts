...
export const TrendminerApi = {
  async fetchJson(path: string, init?: RequestInit) {
    const base = (configs.networks.ae_uat.apiUrl || '').replace(/\/$/, '');
...
async function fetchJson(path: string, init?: RequestInit) {
  const mode = process.env.NODE_ENV || (__DEV__ ? 'development' : 'production');
  const isDevLike = mode === 'development' || mode === 'test';
  const base = (configs.networks.ae_uat.superheroBackendUrl || 'https://raendom-backend.z52da5wt.xyz').replace(/\/$/, '');
...
export const Backend = {
    ...
    getProfileImageUrl: (address: string) => `${configs.networks.ae_uat.superheroBackendUrl}/profile/image/${address}`,
    // Post detail
    ...
    getWordRegistry: (ordering?: string, direction?: string, search?: string) => {
      ...
      return configs.networks.ae_uat.superheroBackendUrl
    ...
    getTipPreviewUrl: (previewLink: string) => `${configs.networks.ae_uat.superheroBackendUrl}${previewLink}`,
